...
export const api = {
  ...
  updateLastSignIn: async (id: string): Promise<void> => {
    try {
        await supabase.from('usuarios').update({ last_sign_in_at: new Date().toISOString() }).eq('id', id);
    } catch (e) {}
  },
  ...
  getAdolescentes: async (): Promise<Adolescente[]> => {
    try {
        ...
        if (error.message?.includes('column "registro" does not exist')) {
            try {
                const basicCols = 'id, nombre, apellido, cedula, fecha_nacimiento, barrio, ciudad, telefono, sexo, estado';
                const data = await fetchAllRows('adolescentes', basicCols, { column: 'nombre', ascending: true });
                return data.map((a: any) => ({ id: a.id, nombre: a.nombre, apellido: a.apellido, cedula: a.cedula, registro: '', fechaNacimiento: a.fecha_nacimiento, barrio: a.barrio || '', ciudad: a.ciudad || '', telefono: a.telefono || '', sexo: a.sexo, estado: a.estado }));
  ...
  getEventos: async (): Promise<Evento[]> => {
    try {
        ...
        return data.map((e: any) => ({ id: e.id, tema: e.tema, lugar: e.lugar, fechaInicio: e.fecha_inicio || e.fechaInicio, horaInicio: e.hora_inicio || e.horaInicio, fechaFin: e.fecha_fin || e.fechaFin, horaFin: e.hora_fin || e.horaFin, tieneCosto: e.tiene_costo || e.tieneCosto, costoTotal: e.costo_total || e.costoTotal, costoPersona: e.costo_persona || e.costoPersona }));
    } catch (error) { return []; }
  },
  ...
  getInscripciones: async (): Promise<InscripcionEvento[]> => {
    ...
    if (error) return [];
    return (data || []).map((i: any) => ({ id: i.id, eventoId: i.evento_id ?? i.eventoId, adolescenteId: i.adolescente_id ?? i.adolescenteId, fechaInscripcion: i.fecha_inscripcion ?? i.fechaInscripcion, notas: i.notas }));
  },

  getPagos: async (): Promise<PagoEvento[]> => {
    const { data, error } = await supabase.from('pagos_eventos').select('*').range(0, MAX_ROWS);
    if (error) return [];
    return (data || []).map((p: any) => ({ id: p.id, inscripcionId: p.inscripcion_id ?? p.inscripcionId, fecha: p.fecha, monto: p.monto }));
  },
  ...
  getParticipantes: async (): Promise<ParticipanteEvento[]> => {
    ...
    if (error) return [];
    return (data || []).map((p: any) => ({ eventoId: p.evento_id ?? p.eventoId, adolescenteId: p.adolescente_id ?? p.adolescenteId }));
  },

  getUsuarios: async (): Promise<Usuario[]> => {
    const { data, error } = await supabase.from('usuarios').select('*');
    if (error) return [];
    return (data || []).map((u: any) => ({ ...u, rolId: u.rol_id ?? u.rolId, avatarUrl: u.avatar_url ?? u.avatarUrl, lastSignInAt: u.last_sign_in_at }));
  },
  ...
  getCumpleanosCelebrados: async (): Promise<CelebracionCumpleanos[]> => {
    ...
    if (error) return [];
    return (data || []).map((c: any) => ({ adolescenteId: c.adolescente_id ?? c.adolescenteId, ano: c.ano }));
  },
  ...
  createAdolescente: async (adolescente: Omit<Adolescente, 'id'>): Promise<Adolescente> => {
    ...
    return { id: result.id, nombre: result.nombre, apellido: result.apellido, cedula: result.cedula, registro: result.registro, fechaNacimiento: result.fecha_nacimiento, barrio: result.barrio || '', ciudad: result.ciudad || '', telefono: result.telefono || '', sexo: result.sexo, estado: result.estado };
  },

  updateAdolescente: async (adolescente: Adolescente): Promise<Adolescente> => {
    // PROTECCIÓN DE PRIMARY KEY: Extraemos el id y enviamos solo el resto de los campos para el UPDATE
    ...
    const { data, error } = await supabase.from('adolescentes').update(dbPayload).eq('id', id).select();
    if (error) throw new Error(error.message);
    if (!data || data.length === 0) throw new Error("No se pudo encontrar el registro para actualizar.");
    
    // Devolvemos el objeto actualizado asegurando que el ID sea el correcto
    const result = data[0];
    return { 
        id: result.id || id, 
  ...
  createEncargado: async (encargado: Omit<Encargado, 'id'>): Promise<Encargado> => {
    ...
    return { id: result.id, nombre: result.nombre, apellido: result.apellido, cedula: result.cedula, fechaNacimiento: result.fecha_nacimiento, barrio: result.barrio, ciudad: result.ciudad, telefono: result.telefono, email: result.email };
  },

  updateEncargado: async (encargado: Encargado): Promise<Encargado> => {
    const { data, error } = await supabase.from('encargados').update({ nombre: encargado.nombre, apellido: encargado.apellido, cedula: encargado.cedula, fecha_nacimiento: encargado.fechaNacimiento || null, barrio: encargado.barrio, ciudad: encargado.ciudad, telefono: encargado.telefono, email: encargado.email || null }).eq('id', encargado.id).select().single();
  ...
  createReunion: async (reunion: Omit<Reunion, 'id'>): Promise<Reunion> => {
    ...
    return { id: result.id, fecha: result.fecha, tema: result.tema, encargadoId: result.encargado_id, estado: result.estado };
  },

  updateReunion: async (reunion: Reunion): Promise<Reunion> => {
    const { data, error } = await supabase.from('reuniones').update({ fecha: reunion.fecha, tema: reunion.tema, encargado_id: reunion.encargadoId, estado: reunion.estado }).eq('id', reunion.id).select().single();
  ...
  createTutor: async (tutor: Omit<Tutor, 'id'>): Promise<Tutor> => {
    ...
    return handleSupabaseData(data, error, 'createTutor');
  },

  updateTutor: async (tutor: Tutor): Promise<Tutor> => {
    const { data, error } = await supabase.from('tutores').update({ nombre: tutor.nombre, apellido: tutor.apellido, cedula: tutor.cedula, telefono: tutor.telefono, parentesco: tutor.parentesco, barrio: tutor.barrio, ciudad: tutor.ciudad }).eq('id', tutor.id).select().single();
    return handleSupabaseData(data, error, 'updateTutor');
  },
  ...
  createTutoresAndLinkBulk: async (tutores: (Omit<Tutor, 'id'> & { adolescenteCedulas: string })[]): Promise<void> => {
    ...
    createdTutores.forEach((nt: any) => {
        ...
        if (input) {
            input.adolescenteCedulas.split(',').map(c => c.trim()).forEach(c => {
  ...
  updateEvento: async (evento: Evento): Promise<Evento> => {
    const { data, error } = await supabase.from('eventos').update({ tema: evento.tema, lugar: evento.lugar, fecha_inicio: evento.fechaInicio, hora_inicio: evento.horaInicio, fecha_fin: evento.fechaFin, hora_fin: evento.horaFin, tiene_costo: evento.tieneCosto, costo_total: evento.costoTotal || null, costo_persona: evento.costoPersona || null }).eq('id', evento.id).select().single();
  ...
  updateInscripcion: async (i: InscripcionEvento): Promise<InscripcionEvento> => {
    const { data, error } = await supabase.from('inscripciones_eventos').update({ evento_id: i.eventoId, adolescente_id: i.adolescenteId, fecha_inscripcion: i.fechaInscripcion, notas: i.notas }).eq('id', i.id).select().single();
